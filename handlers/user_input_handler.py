...
from services.gpt_service import EventHandler, generate_gpt_response, handle_gpt_response
...
async def process_selecting_category(callback_query: CallbackQuery, callback_data: CallbackData, state: FSMContext):
    ...
    if selected:
        ...
        # response_text = await asyncio.create_task(handle_gpt_response(user_name, calculate_values(date.day, date.month, date.year)))
