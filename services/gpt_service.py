...
async def generate_gpt_response_matrix(user_name, values, assistant):
...
async def generate_gpt_response_numerology(user_name, energies, assistant):
    ...
    prompt = (
        ...
        f"Ты должен выдавать только интерпретации арканов в каждом доме, согласно их значению. Объем каждого пункта должен быть 5-6 предложений.\n\n"
    ...
    try:
        ...
        print(f"Error in generate_gpt_response_numerology: {e}")
        return None
